...
- `tests/test_validate_public_repository_safety.py::test_same_inode_mutation_during_read_fails` is intermittently flaky in the cloud VM (~1 in 10 runs). It overwrites a temp file mid-read and expects the validator's before/after `fstat` to observe a changed `st_mtime_ns`/`st_ctime_ns`, but the VM's `/tmp` is `overlayfs` with coarse timestamp granularity (back-to-back same-size rewrites often share an identical `mtime_ns`), so the change is occasionally invisible. This is environment-specific, not a code defect; the suite passes reliably on the project's `macos-14` CI runner (APFS, nanosecond timestamps). Just re-run `python3 -m unittest discover -s tests` if this single test fails.
